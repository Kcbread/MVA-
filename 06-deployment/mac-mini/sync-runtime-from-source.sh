...
SOURCE_APP_DIR="$SOURCE_ROOT/05-engineering-source/procurement-prototype"
SOURCE_DEPLOY_DIR="$SOURCE_ROOT/06-deployment/mac-mini"

RUNTIME_ROOT="${RUNTIME_ROOT:-/Users/kai-chenyang/Services/mva-procurement}"
RUNTIME_APP_DIR="$RUNTIME_ROOT/05-engineering-source/procurement-prototype"
RUNTIME_DEPLOY_DIR="$RUNTIME_ROOT/06-deployment/mac-mini"
...
usage() {
  cat <<'EOF'
Usage: 06-deployment/mac-mini/sync-runtime-from-source.sh
...
This script preserves runtime-only state:
  - 06-deployment/mac-mini/.env
  - 06-deployment/mac-mini/imports
  - uploads
  - logs
  - backups
...
if [[ "$BACKUP_RUNTIME" == "1" && -d "$RUNTIME_APP_DIR" ]]; then
  TS="$(date +%Y%m%d-%H%M%S)"
  BACKUP_DIR="$RUNTIME_ROOT/backups/05-engineering-source/procurement-prototype-before-sync-$TS"
