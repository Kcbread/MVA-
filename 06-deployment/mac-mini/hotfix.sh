...
APP_DIR="$REPO_ROOT/05-engineering-source/procurement-prototype"
...
usage() {
  cat <<'EOF'
Usage: 06-deployment/mac-mini/hotfix.sh [quick|full|deploy|health|status]

quick   Run syntax + unit/contract tests, rebuild from local Docker cache, and health check.
full    Run 05-engineering-source/procurement-prototype/test.sh, rebuild from local Docker cache, and health check.
