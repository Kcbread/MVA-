...
```text
~/mva-procurement/05-engineering-source/procurement-prototype
```

如果用 USB / AirDrop / zip 帶過去，放好後進入專案：

```bash
cd ~/mva-procurement/05-engineering-source/procurement-prototype
npm install
```
...
cd ~/mva-procurement
git clone <repo-url>
cd <repo-folder>/05-engineering-source/procurement-prototype
npm install
```

## `.env` 設定

在 `05-engineering-source/procurement-prototype` 目錄建立 `.env`：
...
若 Mac mini 上的專案已經是 git clone，後續更新建議直接在 `05-engineering-source/procurement-prototype` 目錄執行：

```bash
bash 05-engineering-source/tools/update-mac-mini-runtime.sh
```
...
```bash
INSTALL_DEPS=never bash 05-engineering-source/tools/update-mac-mini-runtime.sh
```
