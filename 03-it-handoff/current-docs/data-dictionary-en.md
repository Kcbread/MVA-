...
| needDate | Scope-level need date for `projectCode + requestLine + demandType`; MFG and Non-MFG may differ | date | Requester |
...
| Field | Meaning | Type | Owner |
| --- | --- | --- | --- |
| requestId | Immutable logical worksheet item row ID; formal IDs use `REQ-{YYMMDD}-{DEPT}-{YEARPROJECT}-{PROJECT}-{NNNN}` | string | System |
...
- `requestId` is generated by the system/server, not entered by Requester.
- Formal request IDs use `REQ-{YYMMDD}-{DEPT}-{YEARPROJECT}-{PROJECT}-{NNNN}`.
- `YYMMDD`, `DEPT`, `YEARPROJECT`, and `PROJECT` are creation/submission snapshots and do not change when later business fields are amended.
- `DEPT` must be a controlled code; `NNNN` is server/DB-generated within the same day + department + yearProject + project scope.
- `requestId` must not include line, MFG/Non-MFG, phase, station, or demand unit; those belong in `stationBreakdown` identity.
- Draft rows may use temporary draft IDs and may be physically removed before submit.
- After submit, `requestId` is immutable. Edits use `revisionNo`, status transitions, and `audit_events`; delete-like actions become cancel / amendment / void status transitions, not hard deletes.
- If the creation snapshot fields such as yearProject, projectCode, or department were fundamentally wrong, cancel/void the original request and create a new `requestId`.
...
| PAS Material No | OM/PAS quote/order context entered during the PAS Quote Result workflow stage in the `Quote Result / Monitor` business screen. | OM/Buyer internal field; must not be shown to Requester. |
