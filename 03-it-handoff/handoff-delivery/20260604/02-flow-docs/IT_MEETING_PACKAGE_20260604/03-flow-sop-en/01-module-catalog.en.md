...
| --- | --- | --- |
| `requester.requestWorkspace` | Request Workspace | Create demand items, add/reuse items, enter Need Date, and submit a package. |
| `requester.demandEditor` | Demand Editor | Enter multiple demand rows under one item. |
| `requester.addReuseItem` | Add / Reuse Item Picker | Search catalog items, create new items, or reuse historical items inside the Request Workspace. |
| `requester.addReuseItem.byItem` | Reuse by Item | Copy one historical item into the current Requester target project/default phase. |
| `requester.addReuseItem.byProjectPackage` | Reuse by Project Package | Preview/import a source project/package set into current draft items with target phase override. |
