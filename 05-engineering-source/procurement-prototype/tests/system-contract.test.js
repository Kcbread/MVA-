...
const requestWorksheetMatrixModule = fs.readFileSync("app-modules/request-worksheet-matrix.js", "utf8");
...
test("Cost Manager review top-level tabs use formal Cost Review naming", () => {
  ...
  assert.match(app, /DEMAND_REVIEW_DENIED/);
  assert.match(app, /DEMAND_REVIEW_REVISE_REQUIRED/);
...
test("Dept DRI, Cost Manager, and Budget Approver use one approval review surface contract", () => {
  ...
  assert.match(html, /data-view="manager" data-roles="dri manager projectDri admin"/);
  assert.match(html, /data-view="priceReview" data-roles="admin"/);
  ...
  assert.match(approvalReviewSurfaceModule, /Approved \/ Denied \/ Revise/);
  assert.doesNotMatch(approvalReviewSurfaceModule, /Final Approve|Approve \/ Reject/);
  ...
  assert.match(app, /updateApprovalReviewState\(managerReviewRole\(currentRole\)/);
  ...
  assert.match(app, /function isManagerReviewRole/);
  assert.match(app, /function applyManagerReviewDecision/);
...
test("Request Tracking is a separate read-only cross-role dashboard surface", () => {
  ...
  assert.match(projectStatusView, /Project[\s\S]*All project codes/);
  assert.doesNotMatch(projectStatusView, /All projects/);
  ...
  assert.match(app, /function projectStatusStageSummaryForUnit/);
  assert.match(app, /project-status-stage-badge/);
  assert.match(app, /function sanitizeProjectStatusMatrixTable/);
  assert.match(app, /removeAttribute\("data-cost-manager-authorization"\)/);
  assert.match(app, /removeAttribute\("data-cost-manager-action"\)/);
  assert.match(app, /projectType:\s*document\.getElementById\("projectStatusProjectTypeFilter"\)/);
  assert.match(app, /projectCode:\s*document\.getElementById\("projectStatusProjectCodeFilter"\)/);
  assert.match(app, /projectTypeForRow\(row\) === filters\.projectType/);
...
test("Cost Manager review embeds protected Demand Analysis baseline instead of duplicate Project Context evidence", () => {
  ...
  assert.match(app, /DEMAND_REVIEW_APPROVED/);
  assert.match(app, /DEMAND_REVIEW_DENIED/);
  assert.match(app, /DEMAND_REVIEW_REVISE_REQUIRED/);
  assert.match(app, /label:\s*"Approved"/);
  assert.doesNotMatch(app, /Final approved/);
...
test("Requester workspace uses MFG / Non-MFG Excel worksheet input", () => {
  ...
  assert.doesNotMatch(departmentView, /<datalist id="projectCodeOptions"/);
  assert.match(app, /function syncProjectCodeInput\(\)[\s\S]*codes\.map\(\(code\) => `<option value="\$\{htmlAttr\(code\)\}">\$\{htmlText\(code\)\}<\/option>`/);
  ...
  assert.match(requestWorksheetMatrixModule, /Hint \/ Action/);
  ...
  assert.match(app, /const G_YEAR_PROJECT_OPTIONS = \["P26 Demo line", "P26", "P26 Zombie line", "P26 Service line", "F27", "P27", "P25 Service line"\]/);
  assert.match(app, /const G_PROJECT_CODE_OPTIONS = \["4CS4", "CGY4", "PKK4", "WGO5", "ASK5", "KSH5", "MCN5", "MT5", "BZ5", "FL5"\]/);
  assert.match(app, /const NON_G_PROJECT_CODE_OPTIONS = \["AS3", "BM2", "LD8", "MA4", "MH2", "ML2", "OR5", "OR6", "SSF"\]/);
  assert.match(app, /function yearProjectOptionsForType/);
  assert.match(app, /function projectCodeOptionsForScope/);
  assert.match(app, /function normalizeYearProjectLabel/);
  assert.match(app, /function normalizeProjectCodeLabel/);
  assert.match(app, /PROJECT_REFERENCE_CODES = \["Bidding list 26"\]/);
  ...
  assert.match(app, /const REQUEST_ACTION_NEW_BUY = "New Buy"/);
  assert.match(app, /const REQUEST_ACTION_OTHER = "Other"/);
  assert.match(app, /function normalizeRequestAction/);
  assert.match(app, /function normalizeRequestIntentFields/);
  assert.match(app, /requestAction,\s*\n\s*requestActionOtherText/);
  assert.match(app, /action: requestAction/);
  assert.match(app, /function requestWorksheetActionControl/);
  assert.match(app, /data-request-action=/);
  assert.match(app, /data-request-action-other=/);
  assert.match(app, /Other action detail is required before submitting this demand scope/);
  assert.match(app, /detailRow\("Requester Action", requestActionValue\(row\)\)/);
  assert.match(app, /detailRow\("Other Action Detail", requestActionOtherTextValue\(row\)/);
  assert.match(app, /detailRow\("Purpose", row\.purpose\)/);
  assert.match(app, /function selectedDemandLineRows\(\)[\s\S]*rowMatchesCurrentRequesterProjectScope\(row\)/);
  assert.match(styles, /\.request-action-select/);
  assert.match(styles, /\.request-action-other-input/);
  ...
  assert.doesNotMatch(app, /row\.project === filter/);
...
test("Need Date and DRI price review layer are wired", () => {
  ...
  assert.match(app, /managerTabs:\s*\["review", "history"\]/);
  assert.match(app, /review:\s*"Dept Review"/);
  assert.match(app, /review:\s*"Budget Review"/);
  ...
  assert.match(html, /id="managerQueue"/);
  ...
  assert.match(priceReviewView, /id="priceReviewQueueTabs"/);
  ...
  const managerReviewView = between(html, '<section class="view" data-view="manager"', '<section class="view" data-view="procurement"');
  assert.match(managerReviewView, /id="managerApprovalQuantityTabs"/);
  assert.match(managerReviewView, /id="managerDemandCostTable"/);
  assert.match(managerReviewView, /id="managerQuantityMatrixTable"/);
  ...
  assert.match(app, /managerTabs: \["review", "history"\]/);
  ...
  assert.match(budgetApproverWorkspaceConfig, /mainViews: \["manager", "projectStatus"\]/);
  assert.match(budgetApproverWorkspaceConfig, /managerTabs: \["review", "history"\]/);
  ...
  assert.match(applyPriceReviewDecision, /const viewportKind = currentView === "manager" \? "manager" : "priceReview"/);
  ...
  assert.match(applyPriceReviewDecision, /Review marked Revise\. Row returned to Requester Action Required/);
...
test("OM tabs and PAS quote result contract are consolidated", () => {
  ...
  assert.match(submissionTable, /<th>Request ID<\/th>\s*<th>Project<\/th>/);
  ...
  assert.doesNotMatch(omView, /data-om-item-scope hidden/);
  ...
  assert.match(app, /element\.hidden = false/);
  ...
  assert.doesNotMatch(app, /function omSubmissionPivotMode/);
  assert.doesNotMatch(app, /function omSubmissionPivotKey/);
  assert.match(app, /function omSubmissionPackageRows/);
  assert.match(app, /function omSubmissionScopeLabel/);
  assert.match(app, /omSubmissionRowKey\(row, filters\)/);
  ...
  assert.match(styles, /\.om-submission-toolbar[\s\S]*grid-template-areas:[\s\S]*"title actions"[\s\S]*"filters filters"[\s\S]*"scope scope"/);
