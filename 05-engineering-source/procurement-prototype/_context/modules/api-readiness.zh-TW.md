...
- Formal `requestId` format: `REQ-{YYMMDD}-{DEPT}-{YEARPROJECT}-{PROJECT}-{NNNN}`.
- `requestId` is server-generated and immutable after submit.
- `YYMMDD` uses creation/submission date; `DEPT` is a controlled department code; `YEARPROJECT` and `PROJECT` are creation/submission snapshots; `NNNN` is generated by server/DB within the same date + department + yearProject + project scope.
- Do not include line, MFG/Non-MFG, phase, station, or demand unit in `requestId`; those remain in stationBreakdown identity.
- Draft rows may use temporary IDs and may be physically deleted before submit.
- Submitted rows must not be hard-deleted. Modify/delete-like behavior must use `revisionNo`, status transitions, and `audit_events`.
- If yearProject / projectCode / department snapshot was fundamentally wrong, cancel/void the original request and create a new `requestId`.
