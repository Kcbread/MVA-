...
- Carryover / Warehouse suggestion appears as row hint/detail, not as an independent add-demand workflow.
...
- Estimate variance is globally visible after OM saves PAS Quote: requester estimate, PAS quote, delta USD/%, total delta, and variance status are visible to all roles, including auto-cleared rows in Request Status/Detail. This does not expose vendor, PAS material no, factory material no, OM assignee, or FTV to requester-facing views, and it does not replace official quote-vs-history price decision routing.
