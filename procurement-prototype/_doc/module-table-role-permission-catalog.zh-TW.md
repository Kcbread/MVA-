...
- `WorkflowStatusModule` 是跨角色狀態的唯一來源；Requester、Cost Owner、OM 可以看不同欄位，但不應各自重新推導 pending owner / current stage / days pending。
- `RoleGuardModule` 是跨角色操作權限的唯一來源；render function 不應散落 `currentRole === ...` 判斷。
- `Cost Owner > Demand Analysis > Cost Dashboard / Station Matrix` 是 protected baseline。可以封裝外殼與 access guard，不可藉由 status/progress refactor 改動欄位、數字邏輯、密度或 drilldown。
...
| `Cost Owner` | Manager B | 高階成本 / P&L 視角；看部門/階段/項目成本、carryover impact、station drilldown | 全域或授權 scope 的 cost dashboard、station matrix、submission monitor | 不做 approve/reject/export；不改 requester demand |
...
| 成本檢視 | Demand Analysis、Cost Dashboard、Station Matrix、carryover impact | Cost Owner, Admin | `canViewCostAnalytics(role, scope)` |
...
| `status-monitor-table` | 進度追蹤 / submission monitor | Requester Request Status、Cost Owner Submission Monitor、OM Submission Dashboard | pending owner、current stage、days pending、next action；不要 PR/PO 百分比當主欄位 | `WorkflowStatusTable`, `TimelineChips`, `PendingOwnerCell` |
| `dense-dashboard-table` | 高密度成本/單位總覽 | Cost Owner Cost Dashboard | Excel-like，可橫向 scroll；數字意義不可被截斷 | `CostDashboardTable`, `DepartmentCostCell`, `CostQtyCell` |
| `matrix-table` | Station Matrix drilldown | Cost Owner Station Matrix | 第二層 detail；保留 Excel-like；只在 table shell 內水平捲動 | `StationMatrixTable`, `MatrixHeader`, `StationQtyCell` |
...
| 表格 / 區塊 | 類型 | 目前用途 | 封裝建議 |
| --- | --- | --- | --- |
| Submission Monitor | `status-monitor-table` | 看卡在哪個 owner、階段、停留天數、風險 | `WorkflowStatusTable(role=costOwner)` |
| Demand Analysis - Cost Dashboard | `dense-dashboard-table` | 高階 cost / unit / phase 總覽 | 已視為 protected module：`CostDashboardTable` |
| Demand Analysis - Station Matrix | `matrix-table` | 第二層 station detail | 已視為 protected module：`StationMatrixTable` |
| Carryover Ledger | `ledger-table` | 常駐看 carryover source / saving / status | `CarryoverLedgerTable(role=costOwner)` |
| Progress Tracking | `status-monitor-table` | 應收斂成 workflow status，不用 PR/PO 百分比當主欄位 | `WorkflowStatusTable(role=costOwner)` |
| Project Setup | `admin-table` / setup | project setup / scope filter | 後續可移到 Admin 或保留 read-only setup |

重要保護：`Cost Dashboard` 與 `Station Matrix` 的 UI 與數字邏輯目前視為 freeze baseline。後續 workflow/status refactor 不得改動其 render output。
...
1. **保護 Cost Owner Demand Analysis**
   - `Cost Dashboard` / `Station Matrix` 先封裝成 protected module。
   - 加 regression guard，確保 workflow/status/table refactor 不改它們的欄位、數字、密度、drilldown。
   - 只允許加 access guard / wrapper / test，不允許趁機改 UI。
...
3. **WorkflowStatusModule**
   ...
   - Cost Owner `Submission Monitor / Progress Tracking`
