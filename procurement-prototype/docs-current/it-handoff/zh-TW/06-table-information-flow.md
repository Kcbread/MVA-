...
| Writes | PAS Material No、vendor、vendor part no、unit price VND input 與 USD canonical price、quote date、quote received date、quote valid until、PDF、Excel、price decision status、estimate-vs-quote variance snapshot、Requester / Dept DRI routing status。 |
...
- Add Item popup 的 `Catalog / Reuse / Copy Demand` 會建立 target worksheet row，target qty 預設 0。`New Item Request` 先建立 pending material master request；完成必填後才進 worksheet。
...
- OM save quote info 會建立全局可視的 estimate variance snapshot：requester estimate、PAS quote、delta USD/%、total delta、variance status；Requester 視圖仍不可暴露 vendor/PAS/factory/FTV 欄位。
