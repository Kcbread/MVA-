...
| 角色 | 測試者 | 用途 |
| --- | --- | --- |
| Requester / OPM DRI | 1 人 | 建立需求、送審、確認 OM quote、cancel / amendment。 |
...
| Budget Approver | 1 人 | 驗證超價或無 history price 的二階審核。 |
...
- Requester 建立的資料，Manager B 與 OM 在不同電腦可同步看到。
- OM Member 儲存 quote 後，OM Leader、Requester、DRI 可看到同一筆更新。
...
| 流程 | 角色順序 | 驗證重點 |
| --- | --- | --- |
| 基本送審到 OM | Requester -> Manager B -> OM | Manager approve 後 OM scope row 進 `PAS Demand No`。 |
