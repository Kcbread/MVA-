...
- 大量 archive、handoff package、07-review-and-artifacts/review-output、截圖、zip 或 generated artifact 刪除必須獨立確認；不要跟功能、部署或 PM memory commit 混在一起。
- 任何會影響未來 thread 的同步/部署規則更新，都要同步更新 `AGENTS.md`、`README.md`、`01-pm-owner/project-progress/MASTER_PM_LEDGER.md`，並在 Notion PM Hub 留下 `Thread Updates` 或 `Decision Log` 紀錄。
...
- GitHub Actions 在每次部署前執行 `05-engineering-source/procurement-prototype/test.sh`。
- 測試通過後才允許部署。
...
- 修改 prototype 後，優先跑 `05-engineering-source/procurement-prototype/test.sh`。
- 回覆必須明確標示：
  - 做了什麼
  - 是否執行測試
  - 是否 commit
...
- Evidence: 本 repo 目前有 `README.md`、`05-engineering-source/procurement-prototype/test.sh` 與既有 Docker handoff runbook 可作為後續部署流程基礎。
