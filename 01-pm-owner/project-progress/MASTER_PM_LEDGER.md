...
- `05-engineering-source/procurement-prototype/_context/` is the first onboarding source for new threads.
...
2. Read `05-engineering-source/procurement-prototype/_context/README.zh-TW.md` for the current role, flow, module, and API context index.
3. If the task touches role, screen, permission, workflow, UI, API, DB, test, deployment, or handoff behavior, identify affected roles and read the relevant role files under `05-engineering-source/procurement-prototype/_context/roles/` before changing behavior.
4. If the task is cross-role, read the relevant flow file under `05-engineering-source/procurement-prototype/_context/flows/`; if it touches tables, modules, API, or persistence, read the relevant file under `05-engineering-source/procurement-prototype/_context/modules/`.
...
7. If the worktree is dirty, unmerged, or already has staged work, identify ownership and read `01-pm-owner/project-progress/WORKTREE_TRIAGE_20260613.md` before editing.
...
10. For material non-trivial work that can be split into independent scopes, consider the multi-agent auto-start rules in `01-pm-owner/project-progress/MULTI_AGENT_WORK_SPEC.md` before doing all work serially in the main thread.
...
`01-pm-owner/project-progress/NEW_THREAD_STARTUP_VALIDATOR.md`. A response is incomplete if
...
- Treat bulk deletions under `archive/`, `03-it-handoff/handoff-delivery/`, `07-review-and-artifacts/review-output/`, `docs-archive/`, `docs-current/IT*`, screenshots, zips, and generated outputs as high-risk until explicitly accepted.
...
- `05-engineering-source/procurement-prototype/_context/README.zh-TW.md`: procurement-specific thread onboarding and handoff rules.
- `01-pm-owner/project-progress/MASTER_PM_LEDGER.md`: repo-local PM memory index.
- `01-pm-owner/project-progress/MULTI_AGENT_WORK_SPEC.md`: multi-agent auto-start, delegation, integration, and receipt rules.
- `01-pm-owner/project-progress/WORKTREE_TRIAGE_20260613.md`: current dirty worktree ownership triage.
