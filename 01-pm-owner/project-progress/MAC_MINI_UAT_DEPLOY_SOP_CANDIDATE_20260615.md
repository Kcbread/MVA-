...
until it is reviewed and promoted into `06-deployment/mac-mini/README.md` or an
...
2. Mac mini runtime secrets live only in `06-deployment/mac-mini/.env`; do not commit it.
...
docker compose --env-file 06-deployment/mac-mini/.env -f 06-deployment/mac-mini/docker-compose.yml up -d --build

sleep 25

curl --noproxy "*" -sS http://127.0.0.1:8080/api/health
docker compose --env-file 06-deployment/mac-mini/.env -f 06-deployment/mac-mini/docker-compose.yml ps
```
...
```zsh
grep -n "P26 Zombie line" /Users/kai-chenyang/Services/mva-procurement-latest/05-engineering-source/procurement-prototype/app.js
grep -n "projectStatusProjectTypeFilter" /Users/kai-chenyang/Services/mva-procurement-latest/05-engineering-source/procurement-prototype/index.html
test -f /Users/kai-chenyang/Services/mva-procurement-latest/05-engineering-source/procurement-prototype/Dockerfile
```

Replace only `05-engineering-source/procurement-prototype`; keep `deploy`, `.env`, uploads, logs, and
backups intact:
...
test -d "$LATEST/05-engineering-source/procurement-prototype"
test -f "$LATEST/05-engineering-source/procurement-prototype/Dockerfile"
test -f "$LATEST/05-engineering-source/procurement-prototype/app.js"
test -f "$LATEST/05-engineering-source/procurement-prototype/index.html"
test -f "$CUR/06-deployment/mac-mini/.env"

mv "$CUR/05-engineering-source/procurement-prototype" "$CUR/05-engineering-source/procurement-prototype-old-$TS"
rsync -a "$LATEST/05-engineering-source/procurement-prototype/" "$CUR/05-engineering-source/procurement-prototype/"

ls -l "$CUR/05-engineering-source/procurement-prototype/Dockerfile"
grep -n "P26 Zombie line" "$CUR/05-engineering-source/procurement-prototype/app.js"
grep -n "projectStatusProjectTypeFilter" "$CUR/05-engineering-source/procurement-prototype/index.html"
echo "$CUR/05-engineering-source/procurement-prototype-old-$TS"
```
...
docker compose --env-file 06-deployment/mac-mini/.env -f 06-deployment/mac-mini/docker-compose.yml exec -T app node -e "const mysql=require('mysql2/promise'); mysql.createConnection({host:process.env.DB_HOST,port:Number(process.env.DB_PORT||3306),user:process.env.DB_USER,password:process.env.DB_PASSWORD,database:process.env.DB_NAME}).then(async c=>{const [r]=await c.query('SELECT 1 AS ok'); console.log(JSON.stringify({ok:true,result:r})); await c.end();}).catch(e=>{console.log(JSON.stringify({ok:false,code:e.code,message:e.message})); process.exit(1);})"
```
...
docker compose --env-file 06-deployment/mac-mini/.env -f 06-deployment/mac-mini/docker-compose.yml down -v
...
- If `docker compose` says `.env` is missing, recreate
  `/Users/kai-chenyang/Services/mva-procurement/06-deployment/mac-mini/.env` from
...
- If `Dockerfile` is missing, the Compose build context likely points to an
  incomplete `05-engineering-source/procurement-prototype` directory. Restore or replace the whole
  `05-engineering-source/procurement-prototype` directory from the latest Git clone.
...
- Decide whether runtime replacement remains allowed or should be replaced by
  `06-deployment/mac-mini/deploy.sh` only.
