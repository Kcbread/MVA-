...
- `06-deployment/mac-mini/deploy.sh` has unresolved `AA` index conflict.
...
- `06-deployment/mac-mini/deploy.sh` must be resolved before any commit or push.

### 6. Archive / Generated Output Cleanup

Scope:

- 285 staged deletions under archive, handoff package, docs-current old packages, docs-archive, 07-review-and-artifacts/review-output, screenshots, and generated artifacts.
...
1. `06-deployment/mac-mini/deploy.sh` is unmerged (`AA`).
...
1. Resolve `06-deployment/mac-mini/deploy.sh`, likely by keeping stage 2 because it includes `curl --noproxy "*"` and `SKIP_DOCKER_PULL`.
...
- Resolve `06-deployment/mac-mini/deploy.sh` with stage 2 or stage 3.
