...
let warehouseStockRecords = [
  { id: "WH-STOCK-001", transactionType: "stock-in", status: "Available", month: "2026-06", item: "Mini PC (Assy)", spec: "LAMITOUCH LM-30 CPU:i3-6100, RAM 16GB DDR4 3200, SSD 256G", qty: 12, ownedQty: 12, sourceProject: "P26", sourceLine: "Line 1", sourceStage: "MP", sourceStation: "CG", sourceRequestId: "WAREHOUSE-P26-MP", createdBy: "Requester", createdAt: "2026-06-03T08:30:00.000Z", reason: "Owned stock remaining from P26 MP CG." },
  { id: "WH-STOCK-002", transactionType: "stock-in", status: "Available", month: "2026-06", item: "Monitor 1", spec: "DELL E2225HM, 21.5 inch HDMI cable", qty: 18, ownedQty: 18, sourceProject: "OR5", sourceLine: "Line 1", sourceStage: "P1.0", sourceStation: "CG", sourceRequestId: "WAREHOUSE-OR5-P10", createdBy: "Requester", createdAt: "2026-06-03T08:45:00.000Z", reason: "Line 1 residual monitor stock." },
  { id: "WH-STOCK-003", transactionType: "stock-in", status: "Available", month: "2026-06", item: "IPC", spec: "DELL Pro Tower QCT1250, Core i3-14100, RAM 8GB DDR5, SSD 512GB", qty: 2, ownedQty: 2, sourceProject: "F27", sourceLine: "Line 1", sourceStage: "P1.0", sourceStation: "CG", sourceRequestId: "WAREHOUSE-F27-L1-CG", createdBy: "Requester", createdAt: "2026-06-05T09:10:00.000Z", reason: "F27 Line 1 has 2 extra CG IPC units." },
  { id: "WH-USE-OR6-IPC-001", transactionType: "use-candidate", status: "Pending Dept DRI", month: "2026-06", item: "IPC", spec: "DELL Pro Tower QCT1250, Core i3-14100, RAM 8GB DDR5, SSD 512GB", qty: 2, sourceProject: "F27", sourceLine: "Line 1", sourceStage: "P1.0", sourceStation: "CG", sourceRequestId: "WAREHOUSE-F27-L1-CG", targetProject: "OR6", targetLine: "Line 2", targetStage: "EVT", targetStationOrUnit: "CG", targetRequestId: "REQ-OR6-L2-IPC", createdBy: "Requester", createdAt: "2026-06-05T09:20:00.000Z", reason: "OR6 Line 2 can use 2 IPC units from F27 Line 1 stock." },
];
...
function itemPickerTargetText(context = itemPickerDemandContext()) {
  ...
  return `Target: ${context.targetProject} / ${context.requestLine} / ${stageLabel(context.targetPhase)} / ${context.demandType}${stationOrUnit ? ` / ${stationOrUnit}` : ""}`;
}
  ...
  root.innerHTML = `
    <section class="request-context-panel" aria-label="Demand input context">
      <div class="request-context-copy">
        ...
        <span>${isMfg ? "MFG: Project / Line → Stage → Station → Item." : "Non-MFG: Project / Line → Stage → Demand Unit → Item."}</span>
      </div>
      <div class="request-context-controls">
        <label>
          <span>Demand Type</span>
          <select id="itemPickerDemandTypeSelect">
            <option value="${DEMAND_TYPE_MFG}" ${isMfg ? "selected" : ""}>MFG · Stage / Station</option>
            <option value="${DEMAND_TYPE_NON_MFG}" ${!isMfg ? "selected" : ""}>Non-MFG · Stage / Unit</option>
          </select>
        </label>
        <label>
        ...
        <label>
          <span>Stage</span>
          <select id="itemPickerStageSelect">
            ${STAGES.map((stage) => `<option value="${stage}" ${stage === context.targetPhase ? "selected" : ""}>${stageLabel(stage)}</option>`).join("")}
          </select>
        </label>
        <label ${isMfg ? "" : "hidden"}>
          <span>Station</span>
      ...
      <div class="request-context-note">${itemPickerTargetText(context)}. Select items below, then enter qty in Selected Demand Lines.</div>
    </section>
  `;
}
...
function openItemPicker() {
  ...
  syncItemPickerDemandContext({ project: requestCarryoverProject(), phase: requestCarryoverPhase(), demandType: lastDemandType });
...
function canLockWarehouseSummary() {
  return ["dri", "deptDri"].includes(currentRole);
}
...
function warehouseTransactionStatus(row = {}) {
  return row.status || (warehouseTransactionType(row) === "stock-in" ? "Available" : "Pending Dept DRI");
}
...
function renderWarehouseMaintenance() {
  ...
  if (summary) {
    summary.innerHTML = summaryCardsHtml([
      ...
      { label: "Pending Dept DRI", value: pending, helper: "Use candidates waiting review", variant: pending ? "warning" : "" },
      ...
      <tr>
        ...
        <td class="cell-action">
          <div class="action-stack">
            ${pendingCandidate && canLockWarehouseSummary() ? `<button class="mini approve" data-warehouse-candidate-lock="${htmlAttr(pendingCandidate.id)}" title="Dept DRI locks this stock-use candidate">Lock</button><button class="mini reject" data-warehouse-candidate-reject="${htmlAttr(pendingCandidate.id)}" title="Reject this stock-use candidate">Reject</button>` : ""}
            ${row.availableQty > 0 ? `<button class="mini approve" data-action="openItemPicker" title="Open Add Demand Lines to create an inventory use candidate">Candidate</button>` : ""}
          </div>
        </td>
        <td class="cell-action">
          <div class="action-stack">
    ...
    ledgerBody.innerHTML = ledgerRows.length ? ledgerRows.map((row) => {
      ...
      return `
        <tr>
          ...
          <td class="cell-action">
            <div class="action-stack">
              ${isPending && canLockWarehouseSummary() ? `<button class="mini approve" data-warehouse-candidate-lock="${htmlAttr(row.id)}">Lock</button><button class="mini reject" data-warehouse-candidate-reject="${htmlAttr(row.id)}">Reject</button>` : ""}
            </div>
          </td>
        </tr>`;
...
function addWarehouseStockRecord() {
  if (currentRole !== "requester") {
    showToast("Only Requester can maintain warehouse stock in this prototype.", "error");
    return;
  }
  ...
  warehouseStockRecords = [{
    ...
    createdAt: new Date().toISOString(),
    reason: "Requester stock-in update.",
  }, ...warehouseStockRecords];
...
function updateWarehouseCandidateStatus(transactionId, decision) {
  if (!canLockWarehouseSummary()) {
    showToast("Only Dept DRI can lock or reject warehouse inventory usage.", "error");
    return;
  }
  ...
  const confirmedBy = roleProfiles[currentRole]?.name || "Dept DRI";
  ...
  updateCarryoverLedgerStatusById(row.carryoverLedgerId, decision === "reject" ? "Rejected" : "Applied", decision === "reject" ? "Dept DRI rejected warehouse inventory use." : "");
  renderWarehouseMaintenance();
  if (typeof renderManagerDemandCostDashboard === "function") renderManagerDemandCostDashboard();
...
function selectedDemandLineRows() {
  return requests.filter((row) => row.project === currentProject && row.status === "Draft" && canRequesterEditRequest(row));
}
  ...
  body.innerHTML = rows.length ? rows.map((row) => {
    ...
    return `
      <tr data-selected-demand-line="${htmlAttr(row.id)}">
        ...
        <td class="cell-number"><input class="compact-input selected-demand-qty" type="number" min="0" step="1" value="${context.qty}" data-selected-demand-qty="${htmlAttr(row.id)}" aria-label="Inline Qty for ${htmlAttr(row.name || "item")}" /></td>
        <td class="cell-date"><input class="compact-date-input" type="date" value="${needDateForRow(row)}" data-selected-demand-need-date="${htmlAttr(row.id)}" /></td>
        <td class="cell-note-summary"><input class="compact-input" type="text" value="${htmlAttr(context.remark)}" placeholder="Remark" data-selected-demand-remark="${htmlAttr(row.id)}" /></td>
        <td class="cell-action">
          <div class="action-stack">
            <button class="mini approve" title="Open full demand breakdown" data-edit-demand="${htmlAttr(row.id)}">Edit</button>
            <button class="mini reject" title="Remove demand line" data-remove-selected-demand="${htmlAttr(row.id)}">Remove</button>
          </div>
        </td>
      </tr>`;
  }).join("") : `<tr><td colspan="6" class="empty-cell">Choose a demand scope, then add catalog, reused, package, or carryover items here.</td></tr>`;
  renderItemPickerCarryoverSuggestions();
}
...
function draftDemandScopeCell(row) {
  ...
  return `
    ...
    <div class="reason-text request-compact-meta">${context.requestLine} · ${context.stationOrUnit}</div>
...
function draftDemandItemCell(row) {
  return `
    ...
    <div class="request-spec-summary cell-spec-clamp" title="${htmlAttr(userVisibleItemDetail(row) || itemDetail(row) || "")}">${userVisibleItemDetail(row) || "-"}</div>`;
}
  ...
  document.getElementById("requestRows").innerHTML = rows.length
    ? rows.map((row) => `
      <tr data-request-row="${row.id}">
        ...
        <td class="cell-text">
          ${draftDemandScopeCell(row)}
        </td>
        <td class="cell-spec">${draftDemandItemCell(row)}</td>
        <td class="cell-date">
    ...
    : `<tr><td colspan="11" class="empty-cell">No draft demand lines yet. Open Add Demand Lines, choose a demand scope, then select catalog, reused, package, or carryover items.</td></tr>`;
}
...
function addRecord(recordId) {
  ...
  renderDepartment();
  renderSelectedDemandLines();
  showToast(`Added to ${targetProject} / ${context.requestLine} / ${stageLabel(targetPhase)}. Enter qty in Selected Demand Lines.`, "success");
}
...
function importHistoryPackage() {
  ...
  const targetProject = context.targetProject;
  const targetPhase = context.targetPhase;
  const imported = sourceRows.map((row) => requestFromRecord(row, historyRequestOverrides(row, true, context)));
  requests = [...imported, ...requests];
  updateRequestCarryover({ project: targetProject, phase: targetPhase, demandType: context.demandType });
  setDeptTab("request");
  renderDepartment();
  renderSelectedDemandLines();
  showToast(`Copied ${imported.length} demand items to ${targetProject} / ${context.requestLine} / ${stageLabel(targetPhase)}.`, "success");
}

function submitRequests() {
  const selectedRows = requests.filter((row) => row.project === currentProject && canRequesterEditRequest(row) && row.selected);
  if (!selectedRows.length) {
    showToast("Select at least one draft request before submitting to Dept DRI.", "error");
    return;
  }
  if (selectedRows.some((row) => !stationBreakdownHasDemand(row))) {
    showToast("Add at least one demand row quantity before submitting to Dept DRI.", "error");
    return;
  }
  if (selectedRows.some((row) => !needDateForRow(row))) {
    showToast("Need Date is required for every selected demand line before submitting.", "error");
    return;
  }
  if (selectedRows.some((row) => !isMaintenanceComplete(row) && !isMaterialNoPending(row))) {
    showToast("Complete material information for selected legacy rows before submitting to Dept DRI.", "error");
    return;
  }
  let submitted = 0;
  const now = new Date().toISOString();
...
function renderMaterialEntryModal() {
  ...
  document.getElementById("materialEntryHelper").textContent = legacy
    ...
    : "Requester does not need to provide a material number. PAS / PUR will provide external material tracking later.";
...
document.addEventListener("change", async (event) => {
  ...
  if ([
    ...
    syncItemPickerDemandContext({
      ...
      phase: document.getElementById("itemPickerStageSelect")?.value || itemPickerStage,
