...
test('action cell contract enforces stacked short-label buttons', () => {
  ...
  assert.match(js, /add item to request/i);
  assert.match(js, /return 'Add'/);
  assert.match(js, /return 'Edit'/);
  assert.match(js, /return 'Remove'/);
  assert.match(js, /return 'Detail'/);
});
...
test('layout contract skips manually managed tables', () => {
  assert.match(indexHtml, /history-item-table" data-layout-managed="manual"/);
  ...
  assert.match(appStyles, /\.history-item-table\[data-layout-managed="manual"\]/);
});
